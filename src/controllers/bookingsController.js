...
exports.getAllBookings = async (req, res) => {
  try {
    ...
    b.ride_status,
    b.transaction_id,
    b.amount,         -- Fetch amount from bookings table
...
exports.createBooking = async (req, res) => {
  const { booking_date, booking_time, cab_id, driver_id, user_id, status } =
    req.body;
...
exports.updateBooking = async (req, res) => {
  try {
    ...
    const [existingBooking] = await db.query(
      "SELECT * FROM bookings WHERE randomId = ?",
      [randomId]
    );
    ...
    res
      .status(200)
      .json({
        message: `Booking with Random ID ${randomId} updated successfully`,
      });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
exports.updateRandomId = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "Random ID updated successfully", randomId });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
};
